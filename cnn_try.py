...
class MNISTNetwork():
    ...
    def __init__(self):
        ...
        self.cl1 = CNN.ConvLayer(28, 28, 1, 5, 5, 6, 0, 1, Activators.TanhActivator(),0.02)  # 输入28*28 一通道，滤波器5*5的6个，步长为1，不补零，所以输出为24*24深度6
        ...
        # 初始化构造卷积层：输入宽度、输入高度、通道数、滤波器宽度、滤波器高度、滤波器数目、补零数目、步长、激活器、学习速率
        self.cl2 = CNN.ConvLayer(12, 12, 6, 5, 5, 12, 0, 1, Activators.TanhActivator(),0.02)  # 输入12*12，6通道，滤波器5*5的12个，步长为1，不补零，所以输出为8*8深度12
        # 构造降采样层，参数为输入宽度、高度、通道数、滤波器宽度、滤波器高度、步长
    ...
    def forward(self,onepic):   #池化层不改变深度，只改变宽高  因为池化层的计算是一层一层的提取运算的
        ...
        flinput = self.pl2.output_array.flatten().reshape(-1, 1)  # 转化为列向量  结果为12*4*4=192  a是个矩阵或者数组，a.flatten()就是把a降到一维，默认是按横的方向降
    ...
    def backward(self,onepic,labels):
        ...
        sensitivity_array = self.fl1.delta.reshape(self.pl2.output_array.shape)  # 将误差转化为同等形状  self.pl2.output_array 形状为12*4*4
        ...
        self.cl2.backward(self.pl1.output_array, self.pl2.delta_array,Activators.TanhActivator())  # 计算第二卷积层的输入误差。参数为第二卷积层的 1、输入，2、输出误差，3、激活函数
        ...
        self.cl1.backward1(onepic, self.pl1.delta_array,Activators.TanhActivator())  # 计算第一卷积层的输入误差。参数为第一卷积层的 1、输入，2、输出误差，3、激活函数
...
if __name__ == '__main__':
    ...
    test_data_set = np.array(test_data_set).astype(bool).astype(int)    #可以将图片简化为黑白图片  将数据转化为bool布尔型，True  false  进而转化为0、1数据
    ...
    # =============================迭代训练=============================
    for i in range(100):  #迭代训练10次。每个迭代内，对所有训练数据进行训练，更新（训练图像个数/batchsize）次网络参数
        print('迭代：',i)
        for k in range(train_data_set.shape[0]):  #使用每一个样本进行训练image.shape[0]#图片垂直尺寸image.shape[1]#图片水平尺寸image.shape[2]#图片通道数
