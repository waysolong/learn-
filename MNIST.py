...
def get_training_data_set(num,onerow=False):
    ...
    # print(image_loader.load(onerow))
    label_loader = LabelLoader('train-labels.idx1-ubyte', num)  # 参数为文件路径和加载的样本数量
    return image_loader.load(onerow), label_loader.load()       # 第一个load是调用的load(self,onerow=False):，第二个是def load(self):，从参数可以看出
...
if __name__=="__main__":
    ...
    print(train_labels[12].argmax())  # 打印样本标签   argmax()  取最大值所在的位置编号
